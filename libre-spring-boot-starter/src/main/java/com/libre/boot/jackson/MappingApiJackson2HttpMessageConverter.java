package com.libre.boot.jackson;
...
public class MappingApiJackson2HttpMessageConverter extends AbstractReadWriteJackson2HttpMessageConverter {
	...
	public MappingApiJackson2HttpMessageConverter(ObjectMapper objectMapper, LibreJacksonProperties properties) {
		super(objectMapper, initWriteObjectMapper(objectMapper, properties), initMediaType(properties));
	}

	private static List<MediaType> initMediaType(LibreJacksonProperties properties) {
	...
	private static ObjectMapper initWriteObjectMapper(ObjectMapper readObjectMapper, LibreJacksonProperties properties) {
		// 拷贝 readObjectMapper
		...
		if (Boolean.TRUE.equals(properties.getNullToEmpty())) {
			writeObjectMapper.setSerializerFactory(writeObjectMapper.getSerializerFactory().withSerializerModifier(new DefaultBeanSerializerModifier()));
			writeObjectMapper.getSerializerProvider().setNullValueSerializer(DefaultBeanSerializerModifier.NullJsonSerializers.STRING_JSON_SERIALIZER);
		}
